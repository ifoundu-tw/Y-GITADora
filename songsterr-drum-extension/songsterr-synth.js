...
globalThis.SDGSongsterrSynth=(()=>{
  ...
  let context,liveSynth,liveNode,renderer,rendererSoundfontId,outputBus,limiter,readyPromise,state='idle',lastError='',songKey='',songState='none',samplePools=new Map();
  ...
  async function ensure(){
    ...
    readyPromise=(async()=>{
      ...
      if(!outputBus){outputBus=context.createGain();limiter=context.createDynamicsCompressor();limiter.threshold.value=-1.5;limiter.knee.value=1;limiter.ratio.value=20;limiter.attack.value=.001;limiter.release.value=.045;outputBus.connect(limiter).connect(context.destination)}
  ...
  return{ensure,loadSong,play,getState:()=>({state,songState,songKey,error:lastError,url:FONT_URL,pools:samplePools.size})};
})();
